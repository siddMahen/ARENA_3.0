...
arr6 = einops.rearrange(arr, "(b b2) c h w -> c (b h) (b2 w)", b2=3)
display_array_as_img(arr6)

# %%
arr7 = einops.repeat(arr[1], "c h w -> c w h")
display_array_as_img(arr7)

# %%

arr8 = einops.reduce(arr, "(b1 b2) c (h dh) (w dw) -> c (b1 h) (b2 w)", "max", b1=2, dh=2, dw=2)
...
def temperatures_differences(temps: Tensor) -> Tensor:
    ...
    """
    assert len(temps) % 7 == 0
    x = einops.rearrange(temps, "(w s) -> w s", w=(len(temps)//7), s=7)  - temperatures_average(temps).unsqueeze(1)
...
def temperatures_normalized(temps: Tensor) -> Tensor:
    ...
    std_dev = einops.reduce(temps, "(w 7) -> w", t.std)
    return (temps - einops.repeat(avg, "w -> (w 7)"))/(einops.repeat(std_dev, "w -> (w 7)"))
...
def classifier_accuracy(scores: Tensor, true_classes: Tensor) -> Tensor:
    ...
    Use t.argmax.
    """
    return (t.argmax(scores, dim=1) == true_classes).sum(dim=-1)/scores.shape[0]
...
def total_price_indexing(prices: Tensor, items: Tensor) -> float:
    ...
    """
    #return t.gather(prices, dim=0, index=items).sum(dim=-1)
...
def total_price_gather(prices: Tensor, items: Tensor) -> float:
    ...
    # I already used gather the first time around!
    return t.gather(prices, dim=0, index=items).sum(dim=-1)
...
def batched_logsumexp(matrix: Tensor) -> Tensor:
    ...
    """
    m = t.amax(matrix, 1)
    x = matrix - m.unsqueeze(1)
    return m + t.log(t.sum(t.exp(x),dim=-1))
...
def batched_softmax(matrix: Tensor) -> Tensor:
    ...
    """
    return t.exp(matrix) / t.exp(matrix).sum(-1, keepdim=True)
...
def batched_cross_entropy_loss(logits: Tensor, true_labels: Tensor) -> Tensor:
    ...
    return -1*batched_logsoftmax(logits).gather(1, true_labels.unsqueeze(1)).sum(dim=-1)
